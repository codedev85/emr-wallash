            ...
            <div class="col-lg-12">
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <h6 class="m-0 font-weight-bold text-primary">Update Subscription Plan</h6>
                        </a>
                        <!-- Card Content - Collapse -->
                        <div class="collapse show" id="collapseCardExample5">
                        <div class="card-body">
                       <form action="{{ url('/subscriptions/'.$findSub->id.'/update') }}" method="POST">
                        @csrf
                        <label>Update Subscription</label>

                        <div>
                            <label>Name</label>
                            <input type="text" name="plan" class="form-control" placeholder="Family Plan" value="{{ $findSub->plan }}"/>
                           <span class="text-danger">{{ $errors->first('plan') }} </span>
                        </div>
                        <div>
                            <label>Amount</label>
                           <input type="text" name="amount" class="form-control" placeholder="200" value="{{ $findSub->amount }}" />
                           <span class="text-danger">{{ $errors->first('amount') }}</span>
                        </div>
                         <br>
                     <button class="btn btn-info">Update Plan</button>
                       </form>
                        </div>
                        </div>
                    </div>
